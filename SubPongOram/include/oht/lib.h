...
void Oht<Ksize, Vsize>::build(const PrfKey &prf_key, unsigned jobs) {
  ...
  for (auto &elem : bins1_) {
    if (!tag::check(elem.tag, tag::kTagFiller) &&
        experiment::get_u64(elem.val + experiment::kTimestampDoneOffset) == 0) {
      experiment::put_u64(elem.val + experiment::kTimestampDoneOffset, timestamp);
    }
  }
  for (auto &elem : bins2_) {
    if (!tag::check(elem.tag, tag::kTagFiller) &&
...
void Oht<Ksize, Vsize>::build_pass(std::vector<Elem<Ksize, Vsize>> &bins,
  ...
  bins.resize(kB_ * kZ_);
  // `shrink_to_fit` can cause realloc, which is a bad thing for this large vector.
  // It largely increases the possibility of an error related with `dlfree`.
  // Or say, I no longer see the error after commenting this.
  // bins.shrink_to_fit();
}
