...
namespace pos {
// `ThreadFn::exit` should be useless now.
// We previously added it to ensure workers always exits after the main thread exits.
// But finally we found out that it is another problem causes the thread sync problem.
// However, we left the addtionally state in our experiment,
// so if you find situations that it becomes not working again,
// you may try recovering this.
// src/oht/obl/par_obl_primitives.cpp and src/lib.cpp also have related code which needs handled.
// enum ThreadFn { cmp, stop, exit };
...
inline void imperative_o_sort_thread(T *arr, size_t n, Comparator cmp, int n_threads, int thread_id) {
  ...
  while (true) {
    ...
    pos::cv.wait(lk, [next_iter, thread_id] {
      ...
      // bool exit = pos::state.fn == pos::ThreadFn::exit;
      /*
      if (ready) {
          printf("[t%d] got job\n", thread_id);
      } else {
    ...
    // if (pos::state.fn == pos::ThreadFn::exit) {
    //   return;
    // }
    lk.unlock();
