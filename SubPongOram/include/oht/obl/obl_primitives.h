...
// Fix implicit type casts and remove unused variables.
#pragma once
#include <cstddef>
#include <cstdint>
#include <type_traits>
...
template <typename Iter>
inline void ObliviousCompact(Iter begin, Iter end, uint8_t *tags);
...
struct reverse_cmp {
  Comparator cmp;
  reverse_cmp(Comparator c) : cmp(c) {}
  bool operator()(const T &a, const T &b) {
    return cmp(b, a);
  }
};
...
inline T ObliviousArrayAccessSimd(const T *arr, size_t i, size_t n) {
  ...
  // Gather 8 cache lines at a time
  {
    ...
    size_t step_size = elem_per_cache_line * 8;  // number of elements we can effectively scan per `gather`

    // the index of the gather that will yield the target
    size_t m = i / step_size;

    // offset into the temporary scanning vector register
    size_t vector_offset = (i % step_size) / elem_per_cache_line;
  ...
  // Gather 4 cache lines at a time
  {
    ...
    size_t step_size = elem_per_cache_line * 4;  // number of elements we can effectively scan per `gather`
...
inline void o_sort(T *arr, uint32_t low, uint32_t len, Comparator cmp) {
  if (len > 1) {
    ...
    if (m * 2 == len) {
      ...
      imperative_o_sort(arr + low, m, obl::reverse_cmp<T, Comparator>(cmp));
...
template <typename T>
inline void o_compact(T *arr, uint8_t *tags, uint32_t len) {
  if (len <= 1) {
    return;
  }
  ...
  for (uint32_t i = 0; i < len; i++) {
    bool pred = ObliviousEqual(tags[i], one_u8);
    uint32_t curr_distance = i - ctr;
    uint32_t next_ctr = ctr + 1;
    distances[i] = ObliviousChoose(pred, curr_distance, zero_u32);
    ctr = ObliviousChoose(pred, next_ctr, ctr);
  }
...
inline void ObliviousCompact(Iter begin, Iter end, uint8_t *tags) {
  using value_type = typename std::remove_reference<decltype(*begin)>::type;
  value_type *array = &(*begin);
  return detail::o_compact<value_type>(array, tags, end - begin);
}
