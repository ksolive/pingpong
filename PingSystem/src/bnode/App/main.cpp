...
public:
    NotifyBNode(size_t id, const std::string& config_path, size_t user_num, size_t round_num, size_t parallel_num, const std::string& interface_name, bool use_B_padding, size_t enode_num, size_t bnode_num) {
        // Set id
        id_ = id;
        ...
        listen(ipString + ":" + get_port(js["bnode_addr"][id]), parallel_num);
        ...
        bnode2enode_buffer_ = new uint8_t[2 * bit_bigger_num * pkt_size_];
        bnode2enode_buffer_size_ = 2 * bit_bigger_num * pkt_size_;
    }
        ...
        while (true) {
            ...
            if (mapping_enodeid_pkt_.size() >= enode_num_) {
                ...
                if (!TEST_NETWORK_LATENCY) {
                    ...
                    if (ecall_handle_blocks(global_eid, pkts_p.data(), lens_sum, pkt_size_, bnode2enode_buffer_, bnode2enode_buffer_size_, lens_data, enode_num_) != SGX_SUCCESS)
                    {
...
int main(int argc, char** argv)
{
    ...
    NotifyBNode notify_bnode(docopt_args["--id"].asLong(), docopt_args["--config-path"].asString(), docopt_args["--user-num"].asLong(), docopt_args["--round-num"].asLong(), docopt_args["--parallel-num"].asLong(), docopt_args["--interface-name"].asString(), docopt_args["--use-B"].asBool(), docopt_args["--enode-num"].asLong(), docopt_args["--bnode-num"].asLong());
    notify_bnode.recv_pkts_from_enode_mt();
