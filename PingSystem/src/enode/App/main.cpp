...
class CallData {
private:
    NotifyService::AsyncService* service_;
...
public:
    CallData(NotifyService::AsyncService* service, ServerCompletionQueue* cq) : service_(service), cq_(cq), responder_(&ctx_), status_(CallStatus::CREATE) {
        process();
    }
...
class NotifyENode final {
private:
    // gRPC correlation
    NotifyService::AsyncService service_;
    std::unique_ptr<Server> server_;
    std::vector<std::unique_ptr<NotifyService::Stub>> connections_bnode_;
    ...
    std::vector<PktConfig> padding_pkg_configs;

public:

    NotifyENode(size_t id, const std::string& network_filename, size_t user_num, size_t round_num, size_t parallel_num, const std::string& interface_name, bool use_B_padding, size_t enode_num, size_t bnode_num) :start_handle_threshold_(static_cast<size_t>(1) << (sizeof(size_t) * 8 - 1)) {
        // Set id
        id_ = id;
        padding_pkg_configs = padding_sential(id_, user_num, parallel_num, enode_num);
        ...
        listen(ipString + ":" + get_port(js["enode_addr"][id]), parallel_num);
        ...
        if (id_ == 0) {
            logging.info("Set B = " + std::to_string(get_B(int(std::ceil(1.f  * user_num / enode_num_)), bnode_num_)) + ", raw num = " + std::to_string(int(std::floor(1.f * user_num / enode_num_ / bnode_num_))));
        }
        size_t bit_bigger_num = get_B(user_num, enode_num_);
        bit_bigger_num = bit_bigger_num < (bnode_num_* B_) ? (bnode_num_ * B_) : bit_bigger_num;
        enode2bnode_buffer_ = new uint8_t[2 * bit_bigger_num * pkt_size_ + pkt_size_];
        enode2clt_buffer_ = new uint8_t[2 * bit_bigger_num * (pkt_size_ + sizeof(uint64_t))];  // uint64_t for plain to_R, big enough, B > input pkts
        enode2bnode_buffer_size_ = 2 * bit_bigger_num * pkt_size_ + pkt_size_;
        enode2clt_buffer_size_ = 2 * bit_bigger_num * (pkt_size_ + sizeof(uint64_t));
    }
...
private:
    std::vector<PktConfig> padding_sential(size_t id, size_t user_num, size_t parallel_num, size_t enode_num) {
        float chip = 1.f * user_num / parallel_num;
        std::vector<PktConfig> res;
        PktConfig pkt_config;
        for (size_t idx = 0;idx < parallel_num;++idx) {
            for (size_t k = std::floor(idx * chip);k < std::floor((idx + 1) * chip);++k) {
                if(k % enode_num == id) {
                    pkt_config.from = k;
                    pkt_config.is_sentail = true;
                    pkt_config.notf = 0;
                    // pkt_config.to = get_sent_target(k, 1);;
                    pkt_config.to = k;
                    res.push_back(pkt_config);
                }
            }
        }
        return res;
...
uint32_t get_sent_target(uint32_t my_id, uint32_t sent_mode=1){
    ...
    // std::tuple<uint32_t, uint32_t, uint32_t, bool> res(my_id, frd_id, get_my_index(my_id, frd_id), false);
    return frd_id;
}
    ...
    void add_connect2bnode(const std::string& addr) {
        ...
        connections_bnode_.emplace_back(NotifyService::NewStub(grpc::CreateCustomChannel(addr, grpc::InsecureChannelCredentials(), channel_args)));
        ...
        while (true) {
            ...
            if (mapping_ip_cd_.size() >= start_handle_threshold_) {
                ...
                if (!TEST_NETWORK_LATENCY) {
                    ...
                    std::vector<uint8_t*> pkts_padding;
                    for (auto pkg_config : padding_pkg_configs) {
                        Pkt tmp_pkt;
                        PktHead tmp_pkt_head;
                        tmp_pkt_head.enode_id = id_;
                        tmp_pkt_head.set_dd_id(get_sent_target(pkg_config.to, 1));
                        tmp_pkt_head.from_S = pkg_config.from;
                        tmp_pkt_head.is_sential = (uint8_t)pkg_config.is_sentail;
                        tmp_pkt.head = tmp_pkt_head;
                        tmp_pkt.set_notf(0);
                        uint8_t* tmp_pkt_buffer = new uint8_t[sizeof(tmp_pkt)];
                        memset(tmp_pkt_buffer, 0, sizeof(tmp_pkt_buffer));
                        memcpy(tmp_pkt_buffer, (uint8_t*)(&tmp_pkt), sizeof(tmp_pkt));
                        pkts_padding.push_back(tmp_pkt_buffer);
                    }
                    size_t lens_data[bnode_num_]{};
                    if (ecall_handle_batch(global_eid, pkts_p.data(), pkts_padding.data(), mapping_ip_cd_.size(), padding_pkg_configs.size(), pkt_size_, enode2bnode_buffer_, enode2bnode_buffer_size_, lens_data, bnode_num_, bnode_num_, B_, id_, round_num_, static_cast<size_t>(use_B_padding_)) != SGX_SUCCESS)
                    {
...
int main(int argc, char** argv)
{
    std::map<std::string, docopt::value> docopt_args = docopt::docopt(
        ...
        "Notify Load Balancer");  // version string
    ...
    NotifyENode notify_enode(docopt_args["--id"].asLong(), docopt_args["--config-path"].asString(), docopt_args["--user-num"].asLong(), docopt_args["--round-num"].asLong(), docopt_args["--parallel-num"].asLong(), docopt_args["--interface-name"].asString(), docopt_args["--use-B"].asBool(), docopt_args["--enode-num"].asLong(), docopt_args["--bnode-num"].asLong());
    notify_enode.recv_pkts_from_clt_mt();
    notify_enode.recv_pkts_from_bnode_mt();
