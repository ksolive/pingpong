...
class CallData {
private:
    PingpongService::AsyncService* service_;
...
public:
    CallData(PingpongService::AsyncService* service, ServerCompletionQueue* cq) : service_(service), cq_(cq), responder_(&ctx_), status_(CallStatus::CREATE) {
        process();
    }
...
class PingpongENode final {
private:
    // gRPC correlation
    PingpongService::AsyncService service_;
    std::unique_ptr<Server> server_;
    std::vector<std::unique_ptr<PingpongService::Stub>> connections_bnode_;
...
public:
    double lambert_w0(double x, double epsilon = 1e-8, int max_iter = 100) {
        double w = x;
        double diff;
        for (int i = 0; i < max_iter; ++i) {
            double exp_w = std::exp(w);
            double w_exp_w = w * exp_w;
            diff = (w_exp_w - x) / (exp_w + w_exp_w);
            w -= diff;
    ...
    int GetPaddedReqsPerSuboram(int user_num_, int enode_num_, int bnode_num_) {
        // // get B
        int incoming_reqs = int(std::ceil(1.f * user_num_ / enode_num_));
        int num_suborams = bnode_num_;
        int sec_param = 128;
        double mu = (double) incoming_reqs / (double) num_suborams;
        double alpha = log(((double) num_suborams) * pow(2.0, sec_param));
    ...
    PingpongENode(size_t id, const std::string& network_filename, size_t user_num, size_t round_num, size_t parallel_num, const std::string& interface_name, bool use_B_padding, size_t enode_num, size_t bnode_num) :start_handle_threshold_(static_cast<size_t>(1) << (sizeof(size_t) * 8 - 1)) {
        // Set id
        id_ = id;
        user_num_ = user_num;
        // Load network info
        ...
        pkt_size_ = PKT_SIZE;
        B_ = use_B_padding_ ? GetPaddedReqsPerSuboram(user_num_, enode_num_, bnode_num_) : 1;
        size_t bit_bigger_num = bnode_num_* B_;
        if (id_ == 0) {
            logging.info("Set batch = " + std::to_string(bit_bigger_num) +"Set B = " + std::to_string(B_) + ", raw num = " + std::to_string(int(std::floor(1.f * user_num / enode_num_ / bnode_num_))));
        }
        enode2bnode_buffer_ = new uint8_t[bit_bigger_num * pkt_size_];  // big enough
        enode2clt_buffer_ = new uint8_t[bit_bigger_num * (pkt_size_ + sizeof(uint64_t))];  
...
private:
    ...
    void send_pkt2bnode(size_t round_num_, size_t id, uint64_t enode_id, const std::string& pkt) {
        PayloadWithRes request;
        request.set_reserved(enode_id);
        request.set_data(pkt);
        if (round_num_ > total_round_num_){
            request.set_read_write(true);
        } else {
            request.set_read_write(false);
        }
    ...
    void recv_pkts_from_clt_st(size_t cq_id) {
        ...
        while (true) {
            ...
            if (cd_from_clt->get_req_reserved() != 0) {
                ...
                over_flag = true;
                std::cout << "debug before round" << std::to_string(mapping_ip_cd_.size()) << "  threshold:" << std::to_string(start_handle_threshold_) << std::endl;
            }
            if (mapping_ip_cd_.size() >= start_handle_threshold_){
                ++round_num_;
                logging.info("debug round: " + std::to_string(round_num_));
                if (round_num_ > total_round_num_) {
                    logging.info("Start Read " + std::to_string(round_num_-total_round_num_) + " round [2.load balancer handle and send to b_node]");
                } else {
                    logging.info("Start Write " + std::to_string(round_num_) + " round [2.load balancer handle and send to b_node]");
                }
                ...
                if (!TEST_NETWORK_LATENCY) {
                    ...
                    if (id_ == 0) {
                        ...
                        if (round_num_ == total_round_num_ * 2) {
                    ...
                    for (size_t bnode_id = 0;bnode_id < bnode_num_;++bnode_id) {
                        // std::cout << "debug send out: " << std::to_string(lens[bnode_id]) << std::endl;
                        send_pkt2bnode(round_num_, bnode_id, id_, std::string(enode2bnode_buffer_p, enode2bnode_buffer_p + lens[bnode_id] * pkt_size_));
                    ...
                    for (size_t bnode_id = 0;bnode_id < bnode_num_;++bnode_id) {
                        send_pkt2bnode(round_num_, bnode_id, id_, std::string(enode2bnode_buffer_ + int(std::round(bnode_id * gap)) * pkt_size_, enode2bnode_buffer_ + int(std::round(bnode_id * gap + gap)) * pkt_size_));
                    }
                }
            ...
            if (resp_pkts_from_bnode_.size() >= bnode_num_) {
                ...
                if (!TEST_NETWORK_LATENCY) {
                    ...
                    for (auto& pair : mapping_ip_cd_) {
                        ...
                        // logging.info("debug mapping_ip_cd_: " + std::to_string(pair.first));
                    }
                ...
                // Send back
                for (auto& pair : mapping_ip_cd_) {
                    // logging.info("back_ip: " + std::to_string(pair.first));
...
int main(int argc, char** argv)
{
    std::map<std::string, docopt::value> docopt_args = docopt::docopt(
        ...
        "Message Load Balancer");  // version string
    ...
    PingpongENode Pingpong_enode(docopt_args["--id"].asLong(), docopt_args["--config-path"].asString(), docopt_args["--user-num"].asLong(), docopt_args["--round-num"].asLong(), docopt_args["--parallel-num"].asLong(), docopt_args["--interface-name"].asString(), docopt_args["--use-B"].asBool(), docopt_args["--enode-num"].asLong(), docopt_args["--bnode-num"].asLong());

    Pingpong_enode.recv_pkts_from_clt_mt();
    Pingpong_enode.recv_pkts_from_bnode_mt();
